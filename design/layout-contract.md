...
- 版本：2.1.0
- 對應 token：`design/tokens.json` v2.0.0
...
`[GATE]` **禁用詞清單。** 對外內容不得出現下列宣稱式用語：

| 禁用詞 | 理由 |
| --- | --- |
| 保證 | 客服頁的承諾具契約意涵，非客服單位可片面宣告 |
| 一定 | 絕對化陳述，遇到例外時會反噬 |
| 最便宜 | 比較性宣稱，涉及不實廣告風險 |
| 市場第一 | 同上，且無從查證 |
...
`[GATE]` **頁面清單由 `content/` 決定，不寫死在本契約。** 每一個 `content/<slug>.md` 對應一個 `site/<slug>.html`，多一個少一個都由內容決定。

驗證器據此推導應有的產出，而非比對固定名單。這一條刻意不凍結：頁面組成隨專案而異，寫死在契約裡會讓契約在第一次調整頁面時就失效。

固定產出（與內容無關，永遠要有）：

| 產出檔 | 說明 |
| --- | --- |
| `site/index.html` | 首頁，由所有內容檔彙整 |
| `site/404.html` | 找不到頁面 |
| `site/search-index.json` | 前端搜尋索引 |
| `site/sitemap.xml` | 網站地圖 |
| `site/robots.txt` | 檢索規則 |
| `site/assets/` | 樣式與腳本 |

頁型判定：`slug` 為 `faq` 者為問答頁，其餘為章節頁。
...
- 問題清單列出 `content/faq.md` 的**全部**問題，僅顯示標題並連到 `faq.html` 的對應錨點。
  不固定則數：則數是隨內容變動的值，寫死在契約裡會在第一次增減問題時失效。
